<?php namespace JFusion\Css;
/**
 * Css parcer class to can be used to some extend to add a prefix to a css file to prevent css conflicts
 *
 * @category   JFusion
 * @package    JFusion\Css
 * @subpackage Css
    ...
    function ParseUrl($url) {
	    ...
	    if (function_exists('curl_init')) {
		    ...
		    if ($FileInfo['http_code'] != 200) {
			    throw new \HttpRequestException($FileInfo['http_code'] . ' error for file: ' . $url);
		    }
	    } else {
		    ...
		    if (!empty($fopen_check)) {
			    ...
			    throw new \RuntimeException('CURL_DISABLED');
		    }
	    }
	    ob_end_clean();
